...
spec.test("correctly checks all the supported interfaces", async (ctx) => {
  ...
  const nftokenNonExistingInterface = await nftoken.instance.methods
...
spec.test("throws when trying to mint NFT to 0x0 address", async (ctx) => {
  ...
  const id1 = ctx.get("id1");
  await ctx.reverts(
    () => nftoken.instance.methods.mint(zeroAddress, id1).send({ from: owner }),
    "003001"
  );
});
...
spec.test("throws when trying to approve NFT ID from a third party", async (ctx) => {
  ...
  await nftoken.instance.methods.mint(bob, id1).send({ from: owner });
  await ctx.reverts(
    () => nftoken.instance.methods.approve(sara, id1).send({ from: sara }),
    "003003"
  );
});
...
spec.test(
  "throws when trying to transfer NFT as an address that is not owner, approved or operator",
  async (ctx) => {
    const nftoken = ctx.get("nfToken");
    const owner = ctx.get("owner");
    const bob = ctx.get("bob");
    ...
    await nftoken.instance.methods.mint(bob, id1).send({ from: owner });
    await ctx.reverts(
      () => nftoken.instance.methods.transferFrom(bob, jane, id1).send({ from: sara }),
      "003004"
    );
  }
);
...
spec.test("throws when trying to transfer NFT to a zero address", async (ctx) => {
  ...
  await nftoken.instance.methods.mint(bob, id1).send({ from: owner });
  await ctx.reverts(
    () => nftoken.instance.methods.transferFrom(bob, zeroAddress, id1).send({ from: bob }),
    "003001"
  );
});
...
spec.test("throws when trying to transfer a invalid NFT", async (ctx) => {
  ...
  await nftoken.instance.methods.mint(bob, id1).send({ from: owner });
  await ctx.reverts(
    () => nftoken.instance.methods.transferFrom(bob, sara, id2).send({ from: bob }),
    "003004"
  );
});
...
spec.test(
  ...
  async (ctx) => {
    ...
    const bobBalance = await nftoken.instance.methods.balanceOf(bob).call();
    const saraBalance = await nftoken.instance.methods
